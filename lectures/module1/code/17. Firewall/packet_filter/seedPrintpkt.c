                       ...
                       const struct nf_hook_state *state)
{
   ...
   if (iph->protocol == IPPROTO_UDP) {
       ...
       if (iph->daddr == ip_addr && ntohs(udph->dest) == port){
            printk(KERN_WARNING "seedPrintpkt: *** Dropping %pI4 (UDP), port %d\n", &(iph->daddr), port);
            return NF_DROP;
        }
   }
   return NF_ACCEPT;
                 ...
                 const struct nf_hook_state *state)
{
   ...
   printk(KERN_INFO "seedPrintpkt: *** %s: ", hook); // Print out the hook info
...
int registerFilter(void) {
   printk(KERN_INFO "seedPrintpkt: *** Registering filters.\n");
...
void removeFilter(void) {
   printk(KERN_INFO "seedPrintpkt: *** The filters are being removed.\n");
